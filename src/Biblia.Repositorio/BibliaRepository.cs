...
namespace Biblia.Repositorio
{
	public class BibliaRepository : RepoBase, IBibliaRepository
	{
		...
		public async Task<IEnumerable<Versiculo>> ObterVersiculosAsync(int versao, int livro, int capitulo)
		{
			using (Conexao)
			{
				try
				{
					...
					return await Conexao.QueryAsync<Versiculo>(query, new { vid = versao, lid = livro, cid = capitulo });
